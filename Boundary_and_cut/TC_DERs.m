...
% parameters for DER(Wind, PV)
%% Problem formulation
% Time period
T = 6;
% beta_V
A_v = [eye(Nbus*T) zeros(Nbus*T,2*T*(Ngen+Nbranch+Ness));
       -eye(Nbus*T) zeros(Nbus*T,2*T*(Ngen+Nbranch+Ness))];
% beta_Generator at non-slack bus 
A_gen_ns = [zeros(Ngen*T,Nbus*T) eye(Ngen*T) zeros(Ngen*T,Ngen*T+2*T*(Nbranch+Ness));
            zeros(Ngen*T,Nbus*T) -eye(Ngen*T) zeros(Ngen*T,Ngen*T+2*T*(Nbranch+Ness));
            zeros(Ngen*T,Nbus*T+Ngen*T) eye(Ngen*T) zeros(Ngen*T,2*T*(Nbranch+Ness));
            zeros(Ngen*T,Nbus*T+Ngen*T) -eye(Ngen*T) zeros(Ngen*T,2*T*(Nbranch+Ness));];% bounds for P Q
...
A_tc = [zeros(2*(T-1)*Ngen,Nbus*T),C_tc,zeros(size(C_tc)),zeros(2*(T-1)*Ngen,2*T*(Nbranch+Ness))];% no ramp for Q
...
A_inj = [C_comb zeros(Nbranch*T,2*T*Ngen) -2*BR_comb -2*BX_comb zeros(Nbranch*T,2*T*Ness);
         -C_comb zeros(Nbranch*T,2*T*Ngen) 2*BR_comb  2*BX_comb zeros(Nbranch*T,2*T*Ness)];

%% beta_ESS
A_aux   = zeros(2*Ness, (Nbus+2*Ngen+2*Nbranch+Ness)*T); % Auxiliary matrix
...
A_ess = [zeros(Ness*T,2*T*(Ngen+Nbranch)+Nbus*T) eye(Ness*T) zeros(Ness*T);
         zeros(Ness*T,2*T*(Ngen+Nbranch)+Nbus*T) -eye(Ness*T) zeros(Ness*T);
         zeros(Ness*T,2*T*(Ngen+Nbranch)+(Nbus+Ness)*T) eye(Ness*T);
         zeros(Ness*T,2*T*(Ngen+Nbranch)+(Nbus+Ness)*T) -eye(Ness*T);
         A_essbe;
         zeros(2*T*Ness,2*T*(Ngen+Nbranch)+Nbus*T) A_E_comb];
...
A_eq = [zeros(Nbus*T) Cg_comb zeros(Nbus*T, Ngen*T) -C_comb2 zeros(Nbus*T, Nbranch*T) Cess_comb zeros(Nbus*T,Ness*T);
        zeros(Nbus*T) -Cg_comb zeros(Nbus*T, Ngen*T) C_comb2 zeros(Nbus*T, Nbranch*T) -Cess_comb zeros(Nbus*T,Ness*T);
        zeros(Nbus*T) zeros(Nbus*T, Ngen*T) Cg_comb zeros(T*Nbus, Nbranch*T) -C_comb2 zeros(T*Nbus, 2*Ness*T);
        zeros(Nbus*T) zeros(Nbus*T, Ngen*T) -Cg_comb zeros(T*Nbus, Nbranch*T) C_comb2 zeros(T*Nbus, 2*Ness*T);];

% Integrate all matrices
A = vertcat(A_v,A_gen_ns,A_tc,A_inj,A_ess,A_eq);
...
b0 = vertcat(b0_U_u,b0_U_l,b0_P_u,b0_P_l,b0_Q_u,b0_Q_l,b0_ramp,b0_inj,b0_ESS,b0_pf);
