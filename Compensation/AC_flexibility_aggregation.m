function [sortedPoints, time] = AC_flexibility_aggregation(mpc,resolution)
    %%Index setting
    % bus idx
    ...
    % power flow equation
    eq_pf          = @(x)AC_power_flow_equation_pol(x(entries_pf{1}),x(entries_pf{2}),...
    ...
    % line Limit
    if any(Fmax)
        ineq_line = @(x)AC_branch_limit_rec(x(entries_pf{1}),...
