...
entries{21} = 10*nb+24*nl+1:10*nb+24*nl+2*Ngen;      %pg_opt, qg_opt
entries{22} = 10*nb+24*nl+2*Ngen+1:11*nb+24*nl+2*Ngen; %theta_u
entries{23} = 11*nb+24*nl+2*Ngen+1:12*nb+24*nl+2*Ngen; %theta_l

%% lower & upper bounds for the variables 
lbx = [vmin;vmin;vmin-v0;vmin-v0;Phi_min;Phi_min;Phi_min-Phi0;Phi_min-Phi0;-inf(6*nb+20*nl+2*Ngen,1);];
ubx = [vmax;vmax;vmax-v0;vmax-v0;Phi_max;Phi_max;Phi_max-Phi0;Phi_max-Phi0; inf(6*nb+20*nl+2*Ngen,1);];
%% initial state x0

x0 = vertcat(v0,v0,zeros(2*nb,1),Phi0,Phi0,zeros(6*nb+22*nl,1),pg0,qg0);
%% constraints
...
cons3 = @(x) create_g_inj(x(entries{9}),x(entries{10}),x(entries{11}),x(entries{12}),x(entries{21}(1:Ngen)),x(entries{21}(Ngen+1:2*Ngen)),Cg,Cl,pl0,ql0,pg_max,pg_min,qg_max,qg_min);
% cons3 = @(x) create_g_inj_slack(x(entries{9}),x(entries{10}),x(entries{11}),x(entries{12}),x(entries{21}(1:Ngen)),x(entries{21}(Ngen+1:2*Ngen)),x(entries{28}(1:Ngen)),x(entries{28}(Ngen+1:2*Ngen)),Cg,Cl,pl0,ql0,pg_max,pg_min,qg_max,qg_min);
...
obj_p = @(x) x(entries{21}(1));
obj_q = @(x) x(entries{21}(Ngen+1));
