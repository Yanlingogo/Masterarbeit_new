...
for c1 = -1:1
    for c2 = -1:1
        if c1== 0 && c2 ==0
            ...
            Points_mu(i,:) = [xopt(Nbus+2*Nbranch+2*Ngen+1), obj_q_opt]; 
            i = i+1;
        end
    end
end
...
for c3 = [-1, 1]
    for i = 1:resolution
        ...
        if c3 == -1
            ...
            grid_p_mu(i,1)  = obj_q_opt;
        else
            gridding_p(i,2) = obj_q_opt;
            grid_p_mu(i,2)  = obj_q_opt;
        end
    end
end
...
for c3 = [-1, 1]
    for i = 1:resolution
        ...
        if c3 == -1
            ...
            grid_q_mu(i,1) = xopt2(Nbus+2*Nbranch+2*Ngen+1);
        else
            gridding_q(i,2) = obj_p_opt;
            grid_q_mu(i,2)  = xopt2(Nbus+2*Nbranch+2*Ngen+1);
        end
    end
end
...
% inner AF
Tot_mu = [Points_mu;[s_p_step';s_p_step'],[grid_p_mu(:,1);grid_p_mu(:,2)];...
    [grid_q_mu(:,1);grid_q_mu(:,2)],[s_q_step';s_q_step']];
centroid_mu = mean(Tot_mu, 1);
angles_mu = atan2(Tot_mu(:,2) - centroid_mu(2), Tot_mu(:,1) - centroid_mu(1));
[~, order_mu] = sort(angles_mu);
sortedmu = Tot_mu(order_mu, :);

plot(sortedPoints(:,1), sortedPoints(:,2), '-bo');
