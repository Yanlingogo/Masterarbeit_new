...
Pg         = SX.sym('Pg',Ngen,1);
Qg         = SX.sym('Qg',Ngen,1);
alpha      = SX.sym('alpha',Ngen-1,1);
x          = vertcat(U, Pij, Qij, Pg, Qg, alpha);
%% lower & upper bounds
lbx         = [Umin;-inf(2*Nbranch,1);Pgmin;Qgmin;zeros(2,1)];       
ubx         = [Umax; inf(2*Nbranch,1);Pgmax;Qgmax;ones(2,1)];
%% initial state x0
...
x0      = vertcat(U0, Pij0, Qij0, Pg0, Qg0, 0.5*ones(Ngen-1,1));
%% LinDistFlow
...
% uncertainty 
uncertain  = create_uncertain_load(Pg,alpha,A,e_l,e_u,cov_load,epsl,Pgmax,Pgmin,id_gen_nslack);
%% Problem formulation
gfun = vertcat(pf_p_eq, pf_q_eq, volt_eq, ref_eq, uncertain);
lbg = [zeros(2*Nbus+Nbranch+1,1);1;-inf(2*(Ngen-1),1)];
ubg = [zeros(2*Nbus+Nbranch+1,1);1;zeros(2*(Ngen-1),1)];
% objective
