...
% lbx         = [-inf(size(Umin));-inf(2*Nbranch,1);Pgmin;-inf(size(Qgmin))];       
% ubx         = [inf(size(Umax)); inf(2*Nbranch,1);Pgmax;inf(size(Qgmax))];
% voltage upper bounds: 
% lbx         = [Umin;-inf(2*Nbranch,1);-inf(size(Pgmin));-inf(size(Qgmin))];       
% ubx         = [Umax; inf(2*Nbranch,1);inf(size(Pgmax));inf(size(Qgmax))];
% bound 3
% Umin(:) = -inf;
% Umax(:) = inf;
% Umin(7) = 1.21;
