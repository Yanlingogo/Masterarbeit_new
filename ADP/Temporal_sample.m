...
entries_pf{1} = 1:Nbus*T;                            % vmag*T
entries_pf{2} = (Nbus*T+1):2*Nbus*T;                 % vang*T
entries_pf{3} = (2*Nbus*T+1):(2*Nbus*T+Ngen*T);      % Pg*T
entries_pf{4} = (2*Nbus*T+Ngen*T+1):2*(Nbus+Ngen)*T; % Qg*T 
...
%% lower & upper bounds
Tvmin       = repmat(vmin,T,1);
Tvmax       = repmat(vmax,T,1);
TPgmin      = repmat(Pgmin,T,1);
TPgmax      = repmat(Pgmax,T,1);
TQgmin      = repmat(Qgmin,T,1);
TQgmax      = repmat(Qgmax,T,1);

lbx         = [vmin;-pi*ones(Nbus*T,1);Pgmin;Qgmin];       
ubx         = [vmax; pi*ones(Nbus*T,1);Pgmax;Qgmax];
%% initial state x0
vang0       = repmat(mpc.bus(:,VA)/180*pi,T,1);
vmag0       = repmat(mpc.bus(:,VM),T,1);
U0          = repmat(vmag0.*cos(vang0),T,1);
W0          = repmat(vmag0.*sin(vang0),T,1);
Pg0         = repmat(mpc.gen(:,PG)/baseMVA,T,1);
Qg0         = repmat(mpc.gen(:,QG)/baseMVA,T,1);
x0      = vertcat(vmag0*T, vang0*T, Pg0*T, Qg0*T);
...
pflow = {};
for i = 1:T
    eq_pf_subset = @(x)create_local_power_flow_equation_pol(x(entries_pf{1}((i-1)*Nbus+1:i*Nbus)),x(entries_pf{2}((i-1)*Nbus+1:i*Nbus)),...
        x(entries_pf{3}((i-1)*Ngen+1:i*Ngen)),x(entries_pf{4}((i-1)*Ngen+1:i*Ngen)),Gbus,Bbus,Pd,Qd,Cg);
    pflow{i} = eq_pf_subset;
end
eq_pf = [];
for i = 1:T
    eq_pf = @(x)vertcat(eq_pf,pflow(i));
end
test_pf        = eq_pf(x0);
Npf            = numel(eq_pf(x0));
