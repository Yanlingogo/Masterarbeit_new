...
entries_pf{5} = 2*(Nbus+Ngen)+1:3*Nbus+2*Ngen; % Pd_error
...
Pgmax_f     = Pgmax;
Pgmax(2:3)  = inf;
Pdemin      = [0;-inf(Nbus-1,1)];
Pdemax      = [0;inf(Nbus-1,1)];
lbx         = [vmin;-pi*ones(Nbus,1);Pgmin;Qgmin;Pdemin];       
ubx         = [vmax; pi*ones(Nbus,1);Pgmax;Qgmax;Pdemax];
%% initial state x0
...
x0      = vertcat(vmag0, vang0, Pg0, Qg0, 0.1*Pd);
%% Uncertainty data
load("e_wind1.mat");
load("e_wind2.mat");
% e_wind1 = e_wind3;
e_wind1 = e_wind1*0.9*0.1;
e_wind2 = e_wind2*0.8*0.1;
...
% load data
mu_guessl  = (rand(32,1)*2-1)*0.01;
load_error =0.01*randn(32,1000);
mu_load = mean(load_error,2);
cov_load= cov(load_error');

ep = 0.05;
K_e = sqrt(ep/(1-ep));
A_wind = -1*ones(Ngen-1,1);
A_load = -1*ones(Nbus-1,1);
...
eq_pf          = @(x)create_power_flow_unload(x(entries_pf{1}),x(entries_pf{2}),...
    x(entries_pf{3}),x(entries_pf{4}),x(entries_pf{5}),Gbus,Bbus,Pd,Qd,Cg);
test_pf        = eq_pf(x0);
Npf            = numel(eq_pf(x0));
...
ineq_Cheb      = @(x) create_Chebyshev(x(entries_pf{3}(id_gen_nslack)),x(entries_pf{5}(2:33)),Pgmax_f(id_gen_nslack),...
    K_e,mu_wind,mu_load,A_wind,A_load,cov_wind,cov_load);
% line Limit
if any(Fmax)
