...
% voltage upper bounds: b2
% vmin(33)=vmax(33);
% lbx         = [vmin;-pi*ones(Nbus,1);-inf(size(Pgmin));-inf(size(Qgmin))];       
% ubx         = [vmax;pi*ones(Nbus,1);inf(size(Pgmax));inf(size(Qgmax))];
...
b8= zeros(resolution,1); % store the solution under gridding
eq_sq = @(x) obj_q(x);
...
lower_bound= zeros(resolution,1); % store the solution under gridding
eq_sp = @(x) obj_p(x);
