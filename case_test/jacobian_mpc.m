function [parameters] = jacobian_mpc(mpc,PCC_0)
%% index
...
%% parameters
    ...
    mpc.gen(id_gen_slack,[PG QG]) = PCC_0*mpc.baseMVA;
    mpc = margin_search(mpc);
    ...
    u_0 = [Pg_0;Qg_0];
    s_0 = [p_pcc0,q_pcc0];
    Pf = mpc.branch(:,14)/mpc.baseMVA;
    Qf = mpc.branch(:,15)/mpc.baseMVA;
    L = (Pf.^2+Qf.^2)./U_0(from_bus);
    z_0 = [U_0; Pf; Qf; p_pcc0; q_pcc0; L];
