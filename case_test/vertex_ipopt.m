function [vert,dispatch,time] = vertex_ipopt(mpc)
tic
    %%Index setting
    % bus idx
    ...
    for i = 1:4
        ...
        dispatch{i} = [xopt(Nbus+2*Nbranch+id_gen_nslack);...
            xopt(Nbus+2*Nbranch+Ngen+id_gen_nslack)];
    end
    ...
    while any(H >= 1e-4)
        ...
        for i = 1:size(Normals,1)
            ...
            % H(i) = abs(Normals(i,:)*[obj_p_opt;obj_q_opt]-1);
    ...
    [vert, remaining_index] = removeRedun(vert, 1e-3);
