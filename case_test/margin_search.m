...
[PW_LINEAR, POLYNOMIAL, MODEL, STARTUP, SHUTDOWN, NCOST, COST] = idx_cost;
    ...
    vm = xopt(1:Nbus);
    va = xopt(Nbus+1:2*Nbus);
    
    V = vm.*cos(va)+1i*(vm.*sin(va));
    Sf = V(from_bus) .* conj(Yf * V) * mpc.baseMVA; %% complex power at "from" bus


    mpc.bus(:,VM) = vm;
    mpc.bus(:,VA) = rad2deg(va);
    mpc.gen(:,PG) = xopt(2*Nbus+1:2*Nbus+Ngen)*mpc.baseMVA;
    mpc.gen(:,QG) = xopt(2*Nbus+Ngen+1:2*Nbus+2*Ngen)*mpc.baseMVA;
    mpc.branch(:, [PF, QF]) = [real(Sf) imag(Sf)];
end
