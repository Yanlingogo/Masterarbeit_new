function [mpc] = runpf_cvxr(mpc)
    ...
    for iter = 1:max_iter
        ...
        S_bal = v_cpx.*conj(Y*v_cpx)-Sinj-Cg*alpha*delta_p-(1i*Cg*alpha*delta_q); % balance: 
        f=[real(S_bal);imag(S_bal(id_ngen))];
        ...
        J  = [J1(:,idx_nslack) J2(:,idx_pq) -Cg*alpha zeros(Nbus,1);...
            J3(id_ngen,idx_nslack) J4(id_ngen,idx_pq) zeros(length(id_ngen),1) -Cg(id_ngen,:)*alpha];
        dx = -J\f;

        va(idx_nslack) = va(idx_nslack)+dx(1:Nbus-1);
        vm(idx_pq) = vm(idx_pq)+dx(Nbus:end-2);
        delta_p = delta_p+dx(end-1);
        delta_q = delta_q+dx(end);
    ...
    mpc.gen(:,QG) = qg_inj;
    mpc.cost = gencost(:,1)'*((pg0+alpha.*delta_p)*mpc.baseMVA).^2 + gencost(:,2)'*(pg0+alpha.*delta_p)*mpc.baseMVA + sum(gencost(:,3));
